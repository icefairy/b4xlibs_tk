...
Sub Class_Globals
	...
	Public ocis As CountingInputStream
	Public ocos As CountingOutputStream
	Public response=Null As OkHttpResponse
	Public CookieAutoProcess As Boolean=True
	Private lTotalSize=-1 As Long
...
Public Sub PostBytes2(Link As String, Data() As Byte)
...
Public Sub PostBytes(Link As String, Data() As Byte)
	...
	ins.InitializeFromBytesArray(Data,0,Data.Length)
	lTotalSize=Data.Length
	ocis.Initialize(ins)
	req.InitializePost(Link, ocis,lTotalSize)
	CallSubDelayed2(HttpUtils2Service, "SubmitJob", Me)
End Sub
...
Public Sub PutBytes(Link As String, Data() As Byte)
	...
	lTotalSize=Data.Length
	ocis.Initialize(ins)
	req.InitializePost(Link,ocis,Data.Length)
	CallSubDelayed2(HttpUtils2Service, "SubmitJob", Me)
End Sub
...
Public Sub PostFile2(Link As String, Dir As String, FileName As String)
	Dim length As Int
	If Dir = File.DirAssets Then
		Log("Cannot send files from the assets folder.")
		Return
...
Public Sub GetHeader As Map
	If response<>Null Then Return response.GetHeaders Else Return Null
End Sub
...
Public Sub cancelDownload
	If ocos<>Null And ocos.IsInitialized Then
		ocos.Close
	End If
	lTotalSize=-1
End Sub

Public Sub GetTotalSize As Long
	If lTotalSize>0 Then Return lTotalSize
	If response<>Null Then Return response.ContentLength
	Return -1
End Sub

Public Sub GetCurrentCount As Long
	If ocis<>Null And ocis.IsInitialized Then Return ocis.Count
	If ocos<>Null And ocos.IsInitialized Then Return ocos.Count
	Return -1
End Sub

Public Sub cancelUpload
	If ocis<>Null And ocis.IsInitialized Then
		ocis.Close
	End If
	lTotalSize=-1
End Sub
