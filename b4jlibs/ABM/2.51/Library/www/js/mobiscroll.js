    ...
    function Scroller(elm, dw, settings) {
        ...
        */
        this.setValue = function (input) {
            ...
            var v = this.returnFormatResult();
            this.val = v;
            this.values = this.temp.slice(0);
            if (input && $(elm).is(':input')) {
				$(elm).val(v).change();	
				var currv = $(elm).attr('returnvalue');				
				$(elm).attr("returnvalue", this.getDate());		

				//var mom1 = this.getDateLocal();
				//console.log(mom1);
				
				var newv = $(elm).attr('returnvalue');
				if (newv!=currv && currv!='0') {
					b4j_raiseEvent('page_parseevent', {'eventname': $(elm).attr('eventname') + '_changed','eventparams': 'datemilliseconds', 'datemilliseconds': newv});
				}
				var currvISO = $(elm).attr('returnvalueISO');				
				$(elm).attr("returnvalueISO", this.getDateISO());				
				var newvISO = $(elm).attr('returnvalueISO');
				if (newvISO!=currvISO && currvISO!='') {
					b4j_raiseEvent('page_parseevent', {'eventname': $(elm).attr('eventname') + '_changediso','eventparams': 'dateISO', 'dateISO': newvISO});
				}
				$(elm).attr("returnvalueISOZ", this.getDateISOZ());								
			} 
        }
        ...
        */
        this.getDate = function () {
            ...
            if (s.preset == 'date') {
                var dd = new Date(d[yOrd], d[mOrd], d[dOrd]);
				return '' + dd.getTime();
			}
            if (s.preset == 'time') {
                var hour = (s.ampm) ? ((d[s.seconds ? 3 : 2] == 'PM' && (d[0] - 0) < 12) ? (d[0] - 0 + 12) : (d[s.seconds ? 3 : 2] == 'AM' && (d[0] == 12) ? 0 : d[0])) : d[0];
                var dd = new Date(1970, 0, 1, hour, d[1], s.seconds ? d[2] : null);
				return '' + dd.getTime();
            }
            if (s.preset == 'datetime') {
                var hour = (s.ampm) ? ((d[s.seconds ? 6 : 5] == 'PM' && (d[3] - 0) < 12) ? (d[3] - 0 + 12) : (d[s.seconds ? 6 : 5] == 'AM' && (d[3] == 12) ? 0 : d[3])) : d[3];
                var dd = new Date(d[yOrd], d[mOrd], d[dOrd], hour, d[4], s.seconds ? d[5] : null);
				/*
				var mom1 = moment(dd).format('YYYY-MM-DD HH:mm:ss');
				console.log(mom1);
				var mom2 = moment(dd).tz('Europe/Paris').format('YYYY-MM-DD HH:mm:ss');
				console.log(mom2);
				var mom3 = moment(dd).tz('America/New_York').format('YYYY-MM-DD HH:mm:ss');
				console.log(mom3);
				*/
				return '' + dd.getTime();
            }
		...
		this.getDateISO = function () {
            ...
            if (s.preset == 'date') {
                var dd = new Date(d[yOrd], d[mOrd], d[dOrd]);
				var mom0 = moment(dd).format('YYYY-MM-DDTHH:mm:ss');							
				return mom0;
			}
            if (s.preset == 'time') {
                var hour = (s.ampm) ? ((d[s.seconds ? 3 : 2] == 'PM' && (d[0] - 0) < 12) ? (d[0] - 0 + 12) : (d[s.seconds ? 3 : 2] == 'AM' && (d[0] == 12) ? 0 : d[0])) : d[0];
		...
		this.getDateISOZ = function () {
            var d = this.values; 
						
            if (s.preset == 'date') {
                var dd = new Date(d[yOrd], d[mOrd], d[dOrd]);
				var mom0 = moment(dd).format('YYYY-MM-DDTHH:mm:ssZ');							
				return mom0;
			}
            if (s.preset == 'time') {
                var hour = (s.ampm) ? ((d[s.seconds ? 3 : 2] == 'PM' && (d[0] - 0) < 12) ? (d[0] - 0 + 12) : (d[s.seconds ? 3 : 2] == 'AM' && (d[0] == 12) ? 0 : d[0])) : d[0];
        ...
        * @param {Boolean} [input] - Also set the value of the associated input element. Default is true.
        */
        this.setDate = function (dd, input) {
			//console.log(dd);
			var d = new Date(parseInt(dd));
			
			/*
			var mom = moment.utc(d);
			console.log(mom);
			var mom = mom.utcOffset(-120).toDate().toUTCString();
			//var mom = moment().utc(d).format('YYYY-MM-DD HH:mm:ssZ');
			console.log(mom);
			*/
			/*
			var mom0 = moment('2016-07-07 05:00:00');
			var mom1 = moment('2016-07-07 05:00:00').tz('America/New_York');
			var mom2 = moment('2016-07-07 05:00:00').tz('Europe/Paris');
			console.log(mom0);
			console.log(mom1);
			console.log(mom2);
			d = mom0.toDate();
		...
		this.setDateISO = function (dd, input) {
			var mom0 = moment(dd);
			var d = mom0.toDate();
        ...
        * @return {String} Formatted string.
        */
        this.returnFormatResult = function () {
            var d = this.temp;
			if (this.preset) {
                if (s.preset == 'date') {
                    return this.formatDate(s.retDateFormat, new Date(d[yOrd], d[mOrd], d[dOrd]), s);
                }
                else if (s.preset == 'datetime') {
                    var hour = (s.ampm) ? ((d[s.seconds ? 6 : 5] == 'PM' && (d[3] - 0) < 12) ? (d[3] - 0 + 12) : (d[s.seconds ? 6 : 5] == 'AM' && (d[3] == 12) ? 0 : d[3])) : d[3];
                    return this.formatDate(s.retDateFormat + ' ' + s.retTimeFormat, new Date(d[yOrd], d[mOrd], d[dOrd], hour, d[4], s.seconds ? d[5] : null), s);
                }
                else if (s.preset == 'time') {
                    var hour = (s.ampm) ? ((d[s.seconds ? 3 : 2] == 'PM' && (d[0] - 0) < 12) ? (d[0] - 0 + 12) : (d[s.seconds ? 3 : 2] == 'AM' && (d[0] == 12) ? 0 : d[0])) : d[0];
                    return this.formatDate(s.retTimeFormat, new Date(1970, 0, 1, hour, d[1], s.seconds ? d[2] : null), s);
                }
            }
            return s.formatResult(d);
        }
    ...
    var dw,
		...
		*/
        ...
        defaults = {
            ...
            preset: 'date',
            dateFormat: 'mm/dd/yy',
			retDateFormat: 'mm/dd/yy',
            dateOrder: 'mmddy',
            ampm: true,
            seconds: false,
            timeFormat: 'hh:ii A',
			retTimeFormat: 'hh:ii A',
        ...
        methods = {
            init: function (options) {
                ...
                switch (options.theme) {
                    ...
                    case 'android-ics light':
                        ...
                        defs.showLabel = false;
                        defs.mode = 'mixed';
						options.theme = 'android-ics light android-ics' + options.abmTheme + ' light' + options.abmTheme;
                        break;
                }
			...
			setDateISO: function(d, input) {
				//console.log(d);
                if (input == undefined) input = false;
                return this.each(function () {
                    if (scrollers[this.id]) {
                        scrollers[this.id].setDateISO(d, input);
                    }
                });
            },
            getDateISO: function() {
                if (scrollers[this[0].id])
                    return scrollers[this[0].id].getDateISO();
            },
			getDateISOZ: function() {
                if (scrollers[this[0].id])
                    return scrollers[this[0].id].getDateISOZ();
            },
            show: function() {
                if (scrollers[this[0].id])
