(function() {
  ...
  callWithJQuery(function($) {
    ...
    var PivotData, addSeparators, aggregatorTemplates, aggregators, dayNamesEn, derivers, getSort, locales, mthNamesEn, naturalSort, numberFormat, pivotTableRenderer, renderers, sortAs, usFmt, usFmtInt, usFmtPct, zeroPad, redvalue, greenvalue, bluevalue, barchartcolor;
    ...
    $.fn.pivotUI = function(input, inputOpts, overwrite, locale) {	
	  ...
	  if (locales[locale]) {
		if (locales[locale].renderersMap) {
			allRenderers = locales[locale].renderersMap;
		}
		if (locales[locale].aggregatorsMap) {
			allAggregators = locales[locale].aggregatorsMap;
		}
	  }
	  ...
	  var colvalues = hexToRgb(opts.heatcol)
	  redvalue = colvalues.r;
	  greenvalue = colvalues.g;
	  bluevalue = colvalues.b;
	  barchartcolor = opts.barchartcol;
      try {
	    attrValues = {};
        materializedInput = [];
        recordsProcessed = 0;
        ...
        renderer = $("<select>").addClass('pvtRenderer').appendTo(rendererControl).bind("change", function() {
		  if (typeof opts.rendererChanged === 'function') {
		     var rend = renderer.val();
			 if (allRenderers) {
			    if (rend in allRenderers) {
			       rend = allRenderers[rend];			
			    }
		     }
			 opts.rendererChanged(rend);			
		  } 
		  return refresh();
        });
		ref = opts.renderers;
		for (x in ref) {
		  ...
		  $("<option>").val(x).html(rend).appendTo(renderer);
        }
		...
		fn = function(attr) {
		  ...
		  */
		  ...
		  var found = $.inArray(attr, opts.hides) > -1;
		  if (found===false) {
			...
			//attrElem = $("<li>").addClass("hidden axis_" + i).append($("<span>").addClass('pvtAttr').text(c).data("attrName", c)); //.append(triangleLink));
			attrElem = $("<li>").addClass("hidden axis_" + i).append($("<span>").addClass('pvtAttr').text(attr).data("attrName", attr));
		  }
          //attrElem = $("<li>").addClass("axis_" + i).append($("<span>").addClass('pvtAttr').text(attr).data("attrName", attr).append(triangleLink));
          if (hasExcludedItem) {
            attrElem.addClass('pvtFilteredAttribute');
        ...
        if (opts.aggregatorName != null) {	
          var aggr = opts.aggregatorName;
		  if (allAggregators) {
			if (aggr in allAggregators) {
			    aggr = allAggregators[aggr];			
			}
		  }	
		  this.find(".pvtAggregator").val(aggr);
    ...
    $.fn.heatmap = function(scope, opts) {
      var colorGen, heatmapper, i, j, l, n, numCols, numRows, ref, ref1, ref2;
      if (scope == null) {
        scope = "heatmap";
      }
      ...
      colorGen = function(color, min, max) {
        ...
        return function(x) {
          var hex, intensity;
          intensity = (100 - Math.round(100 * (x - min) / (max - min)))/100;
		  return (1-intensity);
        };
		*/
		return function(red,green,blue,x) {
			intensity = (100 - Math.round(100 * (x - min) / (max - min)))/100;
			var alpha = (1-intensity);
			var nred = Math.round((alpha * (red / 255) + (intensity * (255 / 255))) * 255);
			var ngreen = Math.round((alpha * (green / 255) + (intensity * (255 / 255))) * 255);
			var nblue = Math.round((alpha * (blue / 255) + (intensity * (255 / 255))) * 255);
      ...
      heatmapper = (function(_this) {
        return function(scope, color) {
          ...
          return forEachCell(function(x, elem) {
            return elem.css("background-color", colorFor(redvalue, greenvalue,bluevalue,x));
			//var currCol = "rgba(" + redvalue + "," + greenvalue + "," + bluevalue + "," + colorFor(x) + ")";
			//return elem.css("background-color", currCol);
          });
        };
      })(this);
	  var colHeat="red";
      ...
      switch (scope) {
        case "heatmap":
          heatmapper(".pvtVal", colHeat);
          break;
        case "rowheatmap":
         for (i = l = 0, ref1 = numRows; 0 <= ref1 ? l < ref1 : l > ref1; i = 0 <= ref1 ? ++l : --l) {
            heatmapper(".pvtVal.row" + i, colHeat);
          }
          break;
        case "colheatmap":
          for (j = n = 0, ref2 = numCols; 0 <= ref2 ? n < ref2 : n > ref2; j = 0 <= ref2 ? ++n : --n) {
            heatmapper(".pvtVal.col" + j, colHeat);
          }
      }
      heatmapper(".pvtTotal.rowTotal", colHeat);
      heatmapper(".pvtTotal.colTotal", colHeat);
      return this;
    };
        ...
        return function(scope) {
          ...
          return forEachCell(function(x, elem) {
            ...
            wrapper.append($("<div>").css({
              ...
              "right": 0,
              "height": scaler(x) + "%",
              "background-color": barchartcolor
            }));
