/* Specific changes made by Alain Bailleul for ABMaterial  */
function resizeFrame(obj) {
	var scroll = $(window).scrollTop();
	//obj.style.visibility = "hidden";
	if (obj.getAttribute("abmenabled")=='true') {
		$(obj).contents().find('body').css('margin', '8px');
	} else {
		$(obj).contents().find('body').css('margin', '0px');
	}	
	obj.style.height = 'auto';
	if (obj.contentDocument) {
		obj.style.height = obj.contentDocument.documentElement.scrollHeight + 15 + 'px';	
	}
...
$(document).ready(function() {
  ...
  Squire.prototype.checkPresenceinSelection = function(name, format,
    validation) {
    var path = this.getPath(),
      test = (validation.test(path) | this.hasFormat(format));
    if (test==1) {
  ...
  SquireUI = function(options) {
	...
	iframe.id = options.id + '-iframe';
	iframe.name = options.id + '-iframe';
	iframe.setAttribute("abm-menu", options.id + '-menu');
	iframe.setAttribute("scrolling", 'no');
	iframe.setAttribute("frameborder", '0');
	iframe.setAttribute("onload", "resizeFrame(this)");
	iframe.setAttribute("abmenabled", "" + options.isEnabled + "");
	iframe.setAttribute("abmtoolbar", "" + options.hasToolbar + "");
	iframe.className = "abmiframe";
	var div = document.createElement('div');
	div.id = options.id + '-div';
    div.className = 'Squire-UI input-field '; 
	
    $(div).load(options.buildPath + 'Squire-UI.html', function() {
	  $(div).find('#menu').attr('id',options.id + '-menu');
	  $(div).find('#' + options.id + '-menu').addClass('menu' + options.theme);
	  if (!options.hasToolbar || !options.isEnabled) {
		$(div).find('#' + options.id + '-menu').addClass('hide');
	  }	
	  $(div).find("[parent-menu='menu']").each(function() {
        $(this).attr('parent-menu', options.id + '-menu');
		$(this).addClass('item-' + options.id);
      })
	  $(div).find('#abm-bold').attr('id',options.id + '-abm-bold');
	  $(div).find('#abm-italic').attr('id',options.id + '-abm-italic');
	  $(div).find('#abm-underline').attr('id',options.id + '-abm-underline');
	  $(div).find('#abm-link').attr('id',options.id + '-abm-link');
	  $(div).find('#abm-ordered').attr('id',options.id + '-abm-ordered');
	  $(div).find('#abm-quote').attr('id',options.id + '-abm-quote');
	  $(div).find('#makeLink').attr('id', 'makeLink' + '-' + options.id);
	  $(div).find('#insertImage').attr('id', 'insertImage' + '-' + options.id);
	  $(div).find('#insertFont').attr('id', 'insertFont' + '-' + options.id);
	  	  
	  $(div).find('#drop-link').attr('id', 'drop-link' + '-' + options.id);
	  $(div).find('#drop-link-title').attr('id', 'drop-link-title' + '-' + options.id);
	  $(div).find('#drop-link-i').attr('id', 'drop-link-i' + '-' + options.id);
	  $(div).find('#drop-link-btn').attr('id', 'drop-link-btn' + '-' + options.id);
	  
	  $(div).find('#drop-font').attr('id', 'drop-font' + '-' + options.id);
	  $(div).find('#drop-font-title').attr('id', 'drop-font-title' + '-' + options.id);
	  $(div).find('#drop-font-i').attr('id', 'drop-font-i' + '-' + options.id);
	  $(div).find('#drop-font-btn').attr('id', 'drop-font-btn' + '-' + options.id);
	  
	  $(div).find('#drop-image').attr('id', 'drop-image' + '-' + options.id);
	  $(div).find('#drop-image-title').attr('id', 'drop-image-title' + '-' + options.id);
	  $(div).find('#drop-image-i').attr('id', 'drop-image-i' + '-' + options.id);
	  $(div).find('#drop-image-btn').attr('id', 'drop-image-btn' + '-' + options.id);
	  
	  
	  $(div).find('#url').attr('id', 'url' + '-' + options.id);
	  $(div).find('#imageUrl').attr('id', 'imageUrl' + '-' + options.id);
	  $(div).find('#font').attr('id', 'font' + '-' + options.id);
	  
	  $(div).find('#drop-link-i' + '-' + options.id).addClass('quit' + '-' + options.id);
	  $(div).find('#drop-image-i' + '-' + options.id).addClass('quit' + '-' + options.id);
	  $(div).find('#drop-font-i' + '-' + options.id).addClass('quit' + '-' + options.id);
	  $(div).find('#drop-link-btn' + '-' + options.id).addClass('submitLink' + '-' + options.id);
	  $(div).find('#drop-image-btn' + '-' + options.id).addClass('sumbitImageURL' + '-' + options.id);
	  $(div).find('#drop-font-btn' + '-' + options.id).addClass('sumbitFont' + '-' + options.id);
	  
	  $(div).find('#drop-link-title' + '-' + options.id).addClass(options.dropboxTitleColor);
	  $(div).find('#drop-image-title' + '-' + options.id).addClass(options.dropboxTitleColor);
	  $(div).find('#drop-font-title' + '-' + options.id).addClass(options.dropboxTitleColor);
	  $(div).find('#drop-link-i' + '-' + options.id).addClass(options.dropboxTitleColor);
	  $(div).find('#drop-image-i' + '-' + options.id).addClass(options.dropboxTitleColor);
	  $(div).find('#drop-font-i' + '-' + options.id).addClass(options.dropboxTitleColor);
	  $(div).find('#drop-link-title' + '-' + options.id).text(options.dropboxLinkTitle);
	  $(div).find('#drop-image-title' + '-' + options.id).text(options.dropboxImageTitle);
	  $(div).find('#drop-font-title' + '-' + options.id).text(options.dropboxFontSizeTitle);
	  
	  $(div).find('#url' + '-' + options.id).css({'border-bottom': '1px solid ' + options.dropboxLineColor, 'box-shadow': '0 1px 0 0 ' + options.dropboxLineColor});
	  $(div).find('#imageUrl' + '-' + options.id).css({'border-bottom': '1px solid ' + options.dropboxLineColor, 'box-shadow': '0 1px 0 0 ' + options.dropboxLineColor});
	  $(div).find('#font' + '-' + options.id).css({'border-bottom': '1px solid ' + options.dropboxLineColor, 'box-shadow': '0 1px 0 0 ' + options.dropboxLineColor});
	  $(div).find('#url' + '-' + options.id).addClass(options.dropboxTextColor);
	  $(div).find('#imageUrl' + '-' + options.id).addClass(options.dropboxTextColor);
	  $(div).find('#font' + '-' + options.id).addClass(options.dropboxTextColor);
	  
	  $(div).find('#drop-link-btn' + '-' + options.id).addClass(options.dropboxButtonBackColor);
	  $(div).find('#drop-link-btn' + '-' + options.id).addClass(options.dropboxButtonForeColor);
	  	  
	  $(div).find('#drop-image-btn' + '-' + options.id).addClass(options.dropboxButtonBackColor);
	  $(div).find('#drop-image-btn' + '-' + options.id).addClass(options.dropboxButtonForeColor);
	  
	  $(div).find('#drop-font-btn' + '-' + options.id).addClass(options.dropboxButtonBackColor);
	  $(div).find('#drop-font-btn' + '-' + options.id).addClass(options.dropboxButtonForeColor);
	  
	  $(div).find('#drop-link-btn' + '-' + options.id).text(options.dropboxLinkButtonText);
	  $(div).find('#drop-image-btn' + '-' + options.id).text(options.dropboxImageButtonText);
	  $(div).find('#drop-font-btn' + '-' + options.id).text(options.dropboxFontSizeButtonText);
	  
      this.linkDrop = new Drop({
        target: $('#makeLink' + '-' + options.id).first()[0],
        content: $('#drop-link' + '-' + options.id).html(),
        position: 'bottom center',
        openOn: 'click'
      });

      this.linkDrop.on('open', function () {
        $('.quit' + '-' + options.id).click(function () {
          $(this).parent().parent().removeClass('drop-open');
        });

        $('.submitLink' + '-' + options.id).click(function () {
          var editor = iframe.contentWindow.editor;
          editor.makeLink($(this).parent().children('#url' + '-' + options.id).first().val());
          $(this).parent().parent().removeClass('drop-open');
          $(this).parent().children('#url' + '-' + options.id).attr('value', '');
        });
      });

      this.imageDrop = new Drop({
        target: $('#insertImage' + '-' + options.id).first()[0],
        content: $('#drop-image' + '-' + options.id).html(),
        position: 'bottom center',
        openOn: 'click'
      });

      this.imageDrop.on('open', function () {
        $('.quit' + '-' + options.id).unbind().click(function () {
          $(this).parent().parent().removeClass('drop-open');
        });

        $('.sumbitImageURL' + '-' + options.id).unbind().click(function () {
          var editor = iframe.contentWindow.editor;
          url = $(this).parent().children('#imageUrl' + '-' + options.id).first()[0];
          editor.insertImage(url.value);
          $(this).parent().parent().removeClass('drop-open');
          $(this).parent().children('#imageUrl' + '-' + options.id).attr('value', '');
        });

      });
	  
	  this.fontDrop = new Drop({
        target: $('#insertFont' + '-' + options.id).first()[0],
        content: $('#drop-font' + '-' + options.id).html(),
        position: 'bottom center',
        openOn: 'click'
      });
      ...
      this.fontDrop.on('open', function () {
        ...
        $('.sumbitFont' + '-' + options.id).unbind().click(function () {
          var editor = iframe.contentWindow.editor;
          var fontSize =  $(this).parent().children('#font' + '-' + options.id).first()[0].value + 'px';
		  if (fontSize!='px') {
      ...
      $('.item' + '-' + options.id).click(function() {
        ...
        } else if (test.isNotValue('makeLink') | test.isNotValue('insertImage') | test.isNotValue('fontsize')) {
    ...
    iframe.addEventListener('load', function() {
	  ...
	  iframe.contentWindow.editor.addStyles(
          ...
          '}' +
          'blockquote {' +
          '  border-left: 2px solid ' + options.blockQuoteColor + ';' +
          '  margin: 0;' +
          '  padding: 0 10px;' +
          '}'		
	  );
	  
	  $(iframe.contentWindow.document).on('keyup keydown autoresize', function () {
		  resizeFrame(iframe);
	  });
	  ...
	  iframe.contentWindow.editor.addEventListener('focus', function() {
		if (iframe.getAttribute("abmenabled")=='true') {
			$('#' + options.id + '-iframe').css({'border': '2px solid ' + options.activeBorderColor, 'box-shadow': 'inset 0 1px 2px rgba(0,0,0,0.075)'});
		} else {
			//$('#' + options.id + '-iframe').css({'border': '2px solid rgba(0,0,0,0.0)', 'box-shadow': 'inset 0 1px 2px rgba(0,0,0,0.0)'});  
			//$('#' + options.id + '-iframe').css({'border': '2px solid ' + options.inactiveBorderColor, 'box-shadow': 'inset 0 1px 2px rgba(0,0,0,0.075)'});
			//$('#' + options.id + '-iframe').css({'border': '0px solid ' + options.inactiveBorderColor, 'box-shadow': 'inset 0 0px 0px rgba(0,0,0,0)'});
			$('#' + options.id + '-iframe').css({'border': '2px solid rgba(0,0,0,0.0)', 'box-shadow': 'inset 0 1px 2px rgba(0,0,0,0.0)'});
		}  
	  });
	  
	  iframe.contentWindow.editor.addEventListener('blur', function() {
		if (iframe.getAttribute("abmenabled")=='true') {
			$('#' + options.id + '-iframe').css({'border': '2px solid ' + options.inactiveBorderColor, 'box-shadow': 'inset 0 1px 2px rgba(0,0,0,0.075)'});
		} else {
			$('#' + options.id + '-iframe').css({'border': '2px solid rgba(0,0,0,0.0)', 'box-shadow': 'inset 0 1px 2px rgba(0,0,0,0.0)'});  
	  ...
	  iframe.contentWindow.editor.addEventListener('pathChange', function() {
		var editor = iframe.contentWindow.editor;
        var action = $(this).data('action');
	
        test = {
          value: $(this).data('action'),
          testBold: editor.checkPresenceinSelection('bold',
             'B', (/>B\b/)),
          testItalic: editor.checkPresenceinSelection('italic',
             'I', (/>I\b/)),
          testUnderline: editor.checkPresenceinSelection(
            'underline', 'U', (/>U\b/)),
          testOrderedList: editor.checkPresenceinSelection(
            'makeOrderedList', 'OL', (/>OL\b/)),
          testLink: editor.checkPresenceinSelection('makeLink',
            'A', (/>A\b/)),
          testQuote: editor.checkPresenceinSelection(
            'increaseQuoteLevel', 'blockquote', (
              />blockquote\b/)),
          isNotValue: function (a) {return (a == action && this.value !== ''); }
        };

		  if (test.testBold) {
			  $('#' + options.id + '-abm-bold').addClass(options.activeMenuColor);
		  }	else {			 
			  $('#' + options.id + '-abm-bold').removeClass(options.activeMenuColor);			  
		  }
		  if (test.testItalic) {
			  $('#' + options.id + '-abm-italic').addClass(options.activeMenuColor);
		  }	else {			 
			  $('#' + options.id + '-abm-italic').removeClass(options.activeMenuColor);			  
		  }
		  if (test.testUnderline) {
			  $('#' + options.id + '-abm-underline').addClass(options.activeMenuColor);
		  }	else {			 
			  $('#' + options.id + '-abm-underline').removeClass(options.activeMenuColor);			  
		  }
		  if (test.testOrderedList) {
			  $('#' + options.id + '-abm-ordered').addClass(options.activeMenuColor);
		  }	else {			 
			  $('#' + options.id + '-abm-ordered').removeClass(options.activeMenuColor);			  
		  }
		   if (test.testLink) {
			  $('#' + options.id + '-abm-link').addClass(options.activeMenuColor);
		  }	else {			 
			  $('#' + options.id + '-abm-link').removeClass(options.activeMenuColor);			  
		  }
		   if (test.testQuote) {
			  $('#' + options.id + '-abm-quote').addClass(options.activeMenuColor);
		  }	else {			 
			  $('#' + options.id + '-abm-quote').removeClass(options.activeMenuColor);			  
		  }
	  });
	  if (options.isEnabled) {
		$('#' + options.id + '-iframe').css({'border': '2px solid ' + options.inactiveBorderColor, 'background-color': options.backColor, 'box-shadow': 'inset 0 1px 2px rgba(0,0,0,0.075)'});
	  } else {
		$('#' + options.id + '-iframe').css({'border': '2px solid rgba(0,0,0,0.0)', 'background-color': options.backColor, 'box-shadow': 'inset 0 1px 2px rgba(0,0,0,0.0)'});  
	  } 
	  resizeFrame(iframe);
    });
	...
	if (options.arrayName=='') {
		b4j_raiseEvent('page_parseevent', {'eventname': options.realId + '_loaded','eventparams': 'target', 'target': options.realId});
	} else {
		b4j_raiseEvent('page_parseevent', {'eventname': options.arrayName + '_loaded','eventparams': 'target', 'target': options.realId});
	}
