...
(function ($) {
  $.fn.extend({
    tableExport: function (options) {
	  ...
	  } else if (defaults.type == 'abm') {
		var typeData = document.getElementById(defaults.planid).contentWindow['rawType'];
		var sData = document.getElementById(defaults.planid).contentWindow['raw'];
		if (typeData=='csv') {
			try {
				var blob = new Blob([sData], {type: "text/csv;charset=utf-8"});
				saveAs(blob, defaults.fileName + '.csv', true);
			}
			catch (e) {
				downloadFile(defaults.fileName + '.csv', 'data:text/csv;charset=utf-8','\ufeff', sData);
			}
		} else {
			try {
      ...
      function parseString(cell, rowIndex, colIndex) {
        ...
        if (cell !== null) {
          ...
          if (defaults.htmlContent === true) {
		    ...
		    var text = htmlData.replace(/\n/g,'\u2028').replace(/<br\s*[\/]?>/gi, '\u2060').replace('\u25E2', '').replace('\u25B6', '');
			...
			if (result=='' && ($cell.hasClass('pvtRowLabel') || $cell.hasClass('pvtColLabel'))) {
				return "";
			}
