...
Sub Handle(req As ServletRequest, resp As ServletResponse)
	...
	Try
		...
		code.Initialize
		code.Append("try {").Append(CRLF)
		code.Append("mJsonParser parser=new mJsonParser(str);").Append(CRLF)
		'code.Append("parser.Initialize(str)").Append(" "&CRLF)
		Dim root As TreeItem = CreateTreeItem ("")
		If squareBracketFound Then
			code.Append("ArrayList<Object> root=parser.NextArray();").Append(" "&CRLF)
			BuildTree(parser.NextArray, root, code, "root", "", False, "")
		Else
			code.Append("LinkedHashMap<String,Object> root=parser.NextObject();").Append(" "&CRLF)
			BuildTree(parser.NextObject, root, code, "root", "", False, "")
		End If
		code.Append($"}catch (Exception jse){
...
Sub BuildTree(element As Object, parent As TreeItem, code As StringBuilder, _
	...
	If BuildList Then
		'Log(indent)
		code.Append(indent.SubString(sIndent.Length)).Append($"}
            }"$).Append(" "&CRLF)
	End If
End Sub
...
Sub WriteCodeHelper (ObjectType As String, code As StringBuilder, _
	...
	If GetFromMap.Length > 0 Then
		...
		Case "int"
			...
			code.Append($"int ${parentName}=parser.obj2int(${GetFromMap.Replace(".Get",".get")});"$).Append(CRLF)
		Case "double"
			code.Append($"double ${parentName}=parser.obj2double(${GetFromMap.Replace(".Get",".get")});"$).Append(CRLF)
			'code.Append("Dim " & parentName & " As " & ObjectType & " = " & GetFromMap.Replace("Get","GetDefault").Replace(")",",0)")).Append(" "&CRLF)
		Case "string"
			code.Append($"String ${parentName}=parser.obj2str(${GetFromMap.Replace(".Get",".get")});"$).Append(CRLF)
			'code.Append("Dim " & parentName & " As " & ObjectType & " = " & GetFromMap.Replace("Get","GetDefault").Replace(")",","""")")).Append(" "&CRLF)
		Case "map"
			code.Append($"HashMap<String,Object> ${parentName} = (HashMap<String, Object>) ${GetFromMap.Replace(".Get",".get")};"$).Append(CRLF)
		Case "list"
			code.Append($"ArrayList<Object> ${parentName} = (ArrayList<Object>) ${GetFromMap.Replace(".Get",".get")};"$).Append(CRLF)
		Case Else
			Log(ObjectType)
			code.Append($"${ObjectType} ${parentName}=${GetFromMap.Replace(".Get",".get")}.toString();"$).Append(CRLF)
...
'			
'		End Select
		code.Append($"if (${parentName.SubString(3)}!=null&&${parentName.SubString(3)}.size()>0){
                for (int ptri = 0; ptri < ${parentName.SubString(3)}.size(); ptri++) {
                    LinkedHashMap<String,Object> ${parentName}= (LinkedHashMap<String, Object>) ${parentName.SubString(3)}.get(ptri);
"$)
